...
static FString GetMetaDataCodeForObject(const UObject* Object, const TCHAR* SymbolName, const TCHAR* Spaces, bool ignoreClass=false)
{
	...
	if (MetaData && MetaData->Num())
	{
		...
		for (const KVPType& KVP : KVPs)
		{
			if (ignoreClass)
			{
				if (Object->GetOuter()->IsA(UClass::StaticClass()) || Object->GetOuter()->IsA(UScriptStruct::StaticClass()))
				{
					continue;
				}
...
static FString GetMetaDataTableForObject(const UObject* Object, const TCHAR* SymbolName, const TCHAR* Spaces)
{
	TMap<FName, FString>* MetaData = UMetaData::GetMapForObject(Object);
	...
	if (MetaData && MetaData->Num())
	{
		...
		for (const KVPType& KVP : KVPs)
		{
			Result.Logf(TEXT("%s %s %s %s %s %s %s %s\r\n"), *Object->GetOuter()->GetFullName(), seperator,*Object->GetName(), seperator,  *KVP.Key.ToString(), seperator,*(KVP.Value) , seperator);
		}
...
FString FNativeClassHeaderGenerator::PropertyNew(FString& Meta, UProperty* Prop, const FString& OuterString, const FString& PropMacro, const TCHAR* Name, const TCHAR* Spaces, const TCHAR* SourceStruct)
{
	...
	//[ META_INFO_OPT <<meta optimization>>
	Meta += GetMetaDataCodeForObject(Prop, Name, Spaces,true);
	GeneratedPackageMeta+= GetMetaDataTableForObject(Prop, Name, Spaces);
	//META_INFO_OPT ]
...
void FNativeClassHeaderGenerator::ExportNativeGeneratedInitCode(FClass* Class, FUHTStringBuilder& OutFriendText)
{
	...
	// The class itself.
	{
		...
		if (Meta.Len())
		{
//[META_INFO_OPT <<meta_optimization>> remove the code ,and load from 
			...
			GeneratedClassRegisterFunctionText.Logf(TEXT("\t\t\t\tMetaData->LoadClassMetaData(OuterClass->GetName());\r\n"));
	...
	for (int32 i = NativeStructs.Num() - 1; i >= 0; --i)
	{
...

		{
			...
			if (Meta.Len())
			{
				...
				GeneratedStructRegisterFunctionText.Logf(TEXT("\t\t\t\tMetaData->LoadClassMetaData(ReturnStruct->GetName());\r\n"));
...
void FNativeClassHeaderGenerator::ExportGeneratedCPP()
{
	...
	for( int32 FileIdx=0;FileIdx<GeneratedFunctionBodyTextSplit.Num();FileIdx++ )
	{
		...
		//[ META_INFO_OPT <<meta optimization>>
		FString MetaFileName = CppPath; 
		MetaFileName += TEXT(".meta");
		FFileHelper::SaveStringToFile( *GeneratedPackageMeta, *MetaFileName);
		//META_INFO_OPT ]
