...
//[META_INFO_OPT
/***/
void UMetaData::LoadClassMetaData(const FString & ClassName)
{
	...
	UMetaData* Metadata = Package->GetMetaData();
	check(Metadata);
	UStruct * OuterClass =FindObject<UStruct>(Package, *ClassName);
	check(OuterClass);
	TArray<FString > Result; 
	FModuleManager::Get().FindClassMetaData(OuterClass->GetFullName(),Result);
	check(Result.Num() % 3 == 0); 
	for (int i = 0; i < Result.Num(); i += 3)
	{
		UProperty * p = OuterClass->FindPropertyByName(*Result[i]); 
		check(p); 
		SetValue(p, *Result[i + 1], *Result[i + 2]); 
	}
};
//META_INFO_OPT]
