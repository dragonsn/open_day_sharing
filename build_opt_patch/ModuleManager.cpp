...
//[ META_INFO_OPT <<meta optimization>>
void FModuleManager::FindClassMetaData(const FString & className, TArray< FString> & result)
{
	if (this->ClassPropertyMetaTable.Find(className))
	{
		result = this->ClassPropertyMetaTable[className];
	};
}

void FModuleManager::LoadExternalMetaData(FString InModuleFileName)
{
	TArray<FString> FoundFiles;
	FString BinPath=FPaths::GetPath(InModuleFileName);
	if (ScanedDirectory.Find(BinPath))
	{
		return; 
	}
	IFileManager::Get().FindFiles(FoundFiles, *BinPath, TEXT("*.umet"));
	ScanedDirectory.Add(BinPath); 
	ScanedDirectory[BinPath] = FoundFiles; 
	for (int fileIdx = 0; fileIdx < FoundFiles.Num(); fileIdx++)
	{
		FString metaFileName = BinPath + TEXT("/");
		metaFileName= metaFileName +FoundFiles[fileIdx];
		FString s;
		bool loaded = FFileHelper::LoadFileToString(s, *metaFileName);
		TCHAR * seperator = TEXT("<@u@>");;
		if (loaded )
		{
			TArray<FString> result;
			s.ParseIntoArray(result, seperator);
			int nb = result.Num();
			//check(nb % 4 == 0); 
			for (int i = 0; i < nb - 3; i += 4)
			{
				FString class_name = result[i].Trim().TrimTrailing();
				if (!this->ClassPropertyMetaTable.Find(class_name))
				{
					ClassPropertyMetaTable.Add(class_name);
				}
				this->ClassPropertyMetaTable[class_name].Add(result[i + 1].Trim().TrimTrailing());
				this->ClassPropertyMetaTable[class_name].Add(result[i + 2].Trim().TrimTrailing());
				this->ClassPropertyMetaTable[class_name].Add(result[i + 3].Trim().TrimTrailing());
				if (class_name == TEXT("Class /Script/UnrealEd.MaterialEditorInstanceConstant") && result[i + 1] == TEXT("PhysMaterial"))
				{
					UE_LOG(LogModuleManager, Warning, TEXT("found"));
				}
			}
		}
	}
...
TSharedPtr<IModuleInterface> FModuleManager::LoadModuleWithFailureReason(const FName InModuleName, EModuleLoadResult& OutFailureReason, bool bWasReloaded /*=false*/)
{
	...
	else
	{
		...
		else
		{
			...
			{
				if (ModuleInfo->Filename.Contains(TEXT("UnrealEd.dll")))
				{
					UE_LOG(LogModuleManager, Warning, TEXT("FOUDN"));
				}
				if (CheckModuleCompatibility(*ModuleFileToLoad))
				{
					...
					if (ModuleInfo->Handle != NULL)
					{
						//[ META_INFO_OPT <<meta optimization>>
#if WITH_METADATA
						LoadExternalMetaData(ModuleFileToLoad);
#endif
						//META_INFO_OPT ]
